...
<?$ElementID = $APPLICATION->IncludeComponent(
	...
	Array(
		...
		"CART_DETAIL" => $arParams["CART_DETAIL"],
	),
	$component
);?>
...
if(count($arCategoryFilter)>0):
	...
	<?$APPLICATION->IncludeComponent(
		...
		Array(
			"AREA_FILE_SHOW" => "file",
			"PATH" => SITE_DIR."lib/feedback/forms/form_popup_catalog.php"
		)
	);?>
	<!-- /FORM EXPERT -->
<?
$ELEMENT_SORT_FIELD = $SORT = 'popular';
$ELEMENT_SORT_ORDER = $ORDER = 'asc';
...
$arrPopularFilter = Array("!ID" => $ElementID);
?>
<?$APPLICATION->IncludeComponent(
	"bitrix:news.list",
	"catalog_popular",
...
<?$APPLICATION->IncludeComponent(
	...
	"section_links", 
	array(
		"COMPONENT_TEMPLATE" => "projects_detail",
		"IBLOCK_TYPE" => "gs_news",
		"IBLOCK_ID" => $arParams["IBLOCK_ID"],
		"SECTION_ID" => "",
		"SECTION_CODE" => "",
...
<?
$obCache = new CPHPCache;
$strCacheID = "kam000000000000000".$componentPath.LANG.$arParams["IBLOCK_ID"].$ElementID;
	if(($tzOffset = CTimeZone::GetOffset()) <> 0)
		$strCacheID .= "_".$tzOffset;
		$CACHE_TIME = $arParams["CACHE_TIME"];
	if($obCache->StartDataCache($CACHE_TIME, $strCacheID, $componentPath))
	{
		$ipropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues( //\SectionValues(
			$arParams["IBLOCK_ID"],
			$ElementID
		);
		$arSection["IPROPERTY_VALUES"] = $ipropValues->getValues();
		$APPLICATION->SetPageProperty("keywords", $arSection["IPROPERTY_VALUES"]["ELEMENT_META_KEYWORDS"]);
		$APPLICATION->SetPageProperty("description", $arSection["IPROPERTY_VALUES"]["ELEMENT_META_DESCRIPTION"]);
		$obCache->EndDataCache($arSection["IPROPERTY_VALUES"]);
	}
	else
	{
